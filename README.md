...
creates the required RANDeployer resources that will start the VNFs in the selected nodes.
...
The RANDeployer creates all the required resources for VNFs execution and clean
...
2. Network Topology: Description of the network, such as links and the capacity of the link (bandwidth and latency).
...
a token that shall be used to get the placement algorithm status and result through a
HTTP `GET` request.
...
by the network operator, a job with the selected algorithm will be triggered. Once the job
finishes its execution it will send the result to a persistency layer accessible by the
Scheduler Manager.

## Prototype Details

The architecture will be implemented on top of K8S, and for prototyping purposes
